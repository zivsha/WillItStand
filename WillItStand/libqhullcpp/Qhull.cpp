...
, m_externalPoints(other.m_externalPoints)
{
...
void Qhull::runQhull3D(const std::vector<vec3> &points, const char* args)
{
    /*
    RboxPoints rbox;
    std::ostringstream rboxCmd;
    rboxCmd << "3 " << points.size();
    for each (vec3 p in points)
    {
        rboxCmd << " " << p;  
    }
    std::istringstream in(rboxCmd.str());
    rbox.appendPoints(in);
    runQhull(rbox, args);
    */
    m_externalPoints = new PointCoordinates(3);
...
void Qhull::
runQhull(const PointCoordinates &rboxPoints, const char *qhullCommand2)
{
